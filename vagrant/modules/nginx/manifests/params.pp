...
class nginx::params {
    ...
    $nx_logdir = $::kernel ? {
        /(?i-mx: linux|Linux)/ => '/var/log/nginx',
        /(?i-mx: sunos)/ => '/var/log/nginx',
    }

    $nx_pid = $::kernel ? {
        /(?i-mx: linux|Linux)/  => '/var/run/nginx.pid',
        /(?i-mx: sunos)/  => '/var/run/nginx.pid',
    }

    if $::osfamily {
        $nx_daemon_user = $::osfamily ? {
            /(?i-mx: redhat|suse|gentoo|linux|Linux)/ => 'nginx',
            /(?i-mx: debian|Debian)/                   => 'www-data',
        ...
        $nx_daemon_user = $::operatingsystem ? {
            /(?i-mx: debian|ubuntu|Debian)/                                                         => 'www-data',
